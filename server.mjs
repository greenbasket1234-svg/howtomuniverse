...
async function metaFetchLevelInsights(accountId, since, until, level) {
  ...
  return allRows.map(row => ({
    ...
    ...(level === 'ad' ? { adgroupId: row.adset_id || '', adgroupName: row.adset_name || '', adId: row.ad_id || '', adName: row.ad_name || '(이름 없음)' } : {}),
    impressions: Number(row.impressions || 0),
    clicks: Number(row.clicks || 0),
    spend: Number(row.spend || 0),
    dbCount: pickAction(row.actions, LEAD_ACTION_PRIORITY),
...
async function cafe24TestConnection(mallId, clientId, clientSecret) {
  ...
  const data = await res.json();
  if (!res.ok) throw new Error(data.error?.message || `카페24 연결 실패 (${res.status})`);
  return { mallName: data.store?.shop_name || mallId, status: 'connected' };
}

// ── 네이버 스마트스토어 (Commerce API) ───────────────────────────────────────
// 인증: application_id + application_secret → Bearer access_token
// API: https://api.commerce.naver.com/external/
const NAVER_COMMERCE_BASE = 'https://api.commerce.naver.com/external';

async function naverStoreGetAccessToken(clientId, clientSecret) {
  const cred = Buffer.from(`${clientId}:${clientSecret}`).toString('base64');
  const res = await fetch(`${NAVER_COMMERCE_BASE}/v1/oauth2/token`, {
    method: 'POST',
    headers: { 'Authorization': `Basic ${cred}`, 'Content-Type': 'application/x-www-form-urlencoded' },
    body: 'grant_type=client_credentials&type=SELLER',
  });
  const data = await res.json();
  if (!res.ok || !data.access_token) {
    throw new Error(data.message || data.error_description || `네이버 스마트스토어 토큰 발급 실패 (${res.status})`);
  }
  return data.access_token;
}

async function naverStoreFetchDailyStats(clientId, clientSecret, since, until) {
  const token = await naverStoreGetAccessToken(clientId, clientSecret);
  // 채널별 판매 통계: 일별 주문 건수 + 결제금액
  const url = new URL(`${NAVER_COMMERCE_BASE}/v1/seller-stats/channel-groups/all/order-stats`);
  url.searchParams.set('from', since);
  url.searchParams.set('to', until);
  url.searchParams.set('timeUnitType', 'DAY');
  const res = await fetch(url.toString(), {
    headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
  });
  const data = await res.json();
  if (!res.ok) {
    // fallback: 채널그룹 없이 전체 판매 통계 시도
    const url2 = new URL(`${NAVER_COMMERCE_BASE}/v1/seller-stats/order-stats`);
    url2.searchParams.set('from', since);
    url2.searchParams.set('to', until);
    url2.searchParams.set('timeUnitType', 'DAY');
    const res2 = await fetch(url2.toString(), {
      headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
    });
    const data2 = await res2.json();
    if (!res2.ok) throw new Error(data2.message || `네이버 스마트스토어 통계 조회 실패 (${res2.status})`);
    return mapNaverStoreStats(data2);
  }
  return mapNaverStoreStats(data);
}

function mapNaverStoreStats(data) {
  const rows = data.data || data.statList || data.items || [];
  return rows.map(row => ({
    date: row.date || row.baseDate || row.statisticDate || '',
    purchases: Number(row.orderCount || row.payCount || row.purchaseCount || 0),
    revenue: Number(row.paymentAmount || row.totalPaymentAmount || row.saleAmount || 0),
  })).filter(r => r.date);
}

async function naverStoreTestConnection(clientId, clientSecret) {
  const token = await naverStoreGetAccessToken(clientId, clientSecret);
  const res = await fetch(`${NAVER_COMMERCE_BASE}/v1/seller/info`, {
    headers: { 'Authorization': `Bearer ${token}` },
  });
  const data = await res.json();
  if (!res.ok) throw new Error(data.message || `네이버 스마트스토어 연결 실패 (${res.status})`);
  return { sellerName: data.sellerName || data.name || '스토어', status: 'connected' };
}
...
async function toggleAdTargetStatus({ targetType, targetId, channel, advertiserId, targetStatus }) {
  if (channel === 'meta') {
    ...
    // 별도 코드 수정 없이 바로 작동합니다. 성공을 거짓으로 꾸미지 않습니다.
    try {
      ...
      const e = new Error(error instanceof Error ? `Meta에서 상태 변경을 거부했습니다: ${error.message} (현재 연결된 토큰에 ads_management 권한이 없으면 이 오류가 납니다)` : '상태 변경에 실패했습니다.'); e.status = 502; throw e;
    }
  }
  if (channel === 'naver') {
    const tenantId = await getCurrentTenantId();
    ...
    // 네이버 검색광고는 캠페인 > 광고그룹(=광고 세트) > 소재/키워드 4단계 전부 같은 방식
    // (userLock 필드)으로 ON/OFF를 지원합니다.
    const NAVER_TARGET_MAP = {
      campaign: { path: 'campaigns', idField: 'nccCampaignId' },
      adset: { path: 'adgroups', idField: 'nccAdgroupId' },
      creative: { path: 'ads', idField: 'nccAdId' },
      keyword: { path: 'keywords', idField: 'nccKeywordId' },
    };
    const mapping = NAVER_TARGET_MAP[targetType] || NAVER_TARGET_MAP.campaign;
    try {
      await naverApiRequest('PUT', `/ncc/${mapping.path}/${targetId}`, { fields: 'userLock' }, credentials, { [mapping.idField]: targetId, userLock: targetStatus === 'off' });
      addLog({ action: 'campaign_toggle', advertiserId, channel, targetType, targetId, result: 'success', data: { targetStatus } });
      return { ok: true, status: targetStatus };
    } catch (error) {
      addLog({ action: 'campaign_toggle', advertiserId, channel, targetType, targetId, result: 'fail', error: error instanceof Error ? error.message : String(error) });
  ...
  try {
    ...
    if (req.method === 'POST' && pathname === '/api/admin/migrate-to-postgres') {
      ...
      try {
        ...
        // AI 분석 지침 커스텀 프롬프트를 테넌트별로 저장합니다.
        await pgPool.query(`ALTER TABLE tenants ADD COLUMN IF NOT EXISTS ai_guidelines JSONB`);
...
function scheduleSyncResultRetry(tenantId, advertiserId, channel, result) {
  ...
  if (typeof timer.unref === 'function') timer.unref();
    ...
    if (req.method === 'PUT' && pathname === '/api/ai-guidelines') {
      ...
      const guidelines = {
        customRules: Array.isArray(body.customRules) ? body.customRules.map(r => String(r).slice(0, 500)).filter(Boolean) : [],
        campaignTypeRules: body.campaignTypeRules && typeof body.campaignTypeRules === 'object' ? Object.fromEntries(Object.entries(body.campaignTypeRules).map(([k,v]) => [String(k).slice(0,100), String(v).slice(0,1000)])) : {},
        updatedAt: new Date().toISOString(),
      };
      await pgPool.query(`UPDATE tenants SET ai_guidelines=$1 WHERE id=$2`, [JSON.stringify(guidelines), tenantId]);
    ...
    if (req.method === 'GET' && pathname === '/api/metrics/creatives') {
      ...
      // 일별 CTR 추이(trend)를 계산해 각 소재에 추가합니다.
      const _dailyMap=new Map();
      for(const row of source){const k=`${row.advertiserId}|${row.channel}|${row.adId}`;if(!_dailyMap.has(k))_dailyMap.set(k,new Map());const dm=_dailyMap.get(k);const dt=String(row.date||'').slice(0,10);if(dt){const ev=dm.get(dt)||{impressions:0,clicks:0,spend:0};ev.impressions+=metricNumber(row.impressions);ev.clicks+=metricNumber(row.clicks);ev.spend+=metricNumber(row.spend);dm.set(dt,ev);}}
      const rows=Array.from(grouped.values()).map(cur=>{const dm=_dailyMap.get(`${cur.advertiserId}|${cur.channel}|${cur.adId}`);const trend=dm?[...dm.entries()].sort((a,b)=>a[0].localeCompare(b[0])).map(([,v])=>v.impressions>0?parseFloat((v.clicks/v.impressions*100).toFixed(3)):0):[];const spendTrend=dm?[...dm.entries()].sort((a,b)=>a[0].localeCompare(b[0])).map(([,v])=>v.spend):[];return{...withDerived(cur),trend,spendTrend,days:trend.length};}).sort((a,b)=>b.spend-a.spend);
    ...
    // ── 카페24 연결 테스트 ────────────────────────────────────────────────────
    if (req.method === 'POST' && pathname === '/api/integrations/cafe24/test') {
      const body = await readJson(req);
      const mallId = cleanText(body.mallId || '', 80);
      const clientId = cleanText(body.clientId || '', 200);
      const clientSecret = cleanText(body.clientSecret || '', 200);
      if (!mallId || !clientId || !clientSecret) return sendJson(res, 400, { error: '쇼핑몰 ID·Client ID·Secret이 필요합니다.' });
      try {
        const info = await cafe24TestConnection(mallId, clientId, clientSecret);
        return sendJson(res, 200, { ok: true, ...info });
      } catch (e) { return sendJson(res, 502, { error: e?.message || '카페24 연결에 실패했습니다.' }); }
    }
    ...
    if (req.method === 'POST' && pathname === '/api/integrations/sync-cafe24') {
      ...
      const since = sinceDate.toISOString().slice(0, 10);
      try {
        const rows = await cafe24FetchDailyOrders(mallId, clientId, clientSecret, since, until);
        if (rows.length) {
          await pgPool.query(
            `INSERT INTO daily_metrics (tenant_id, advertiser_id, channel, date, impressions, clicks, spend, db_count, purchases, revenue, add_to_cart, complete_registration, initiate_checkout, unconfirmed_count)
             SELECT $1,$2,'cafe24',d,0,0,0,0,pur,rev,0,0,0,0
             FROM UNNEST($3::date[],$4::bigint[],$5::numeric[]) AS t(d,pur,rev)
             ON CONFLICT (advertiser_id, channel, date) DO UPDATE SET
               purchases=EXCLUDED.purchases, revenue=EXCLUDED.revenue, updated_at=now()`,
            [tenantId, advertiserId, rows.map(r=>r.date), rows.map(r=>r.purchases), rows.map(r=>r.revenue)]
          );
        }
        await pgPool.query(`UPDATE media_accounts SET last_synced_at=now(), last_row_count=$1, status='connected' WHERE tenant_id=$2 AND advertiser_id=$3 AND channel='cafe24'`, [rows.length, tenantId, advertiserId]);
        return sendJson(res, 200, { ok: true, days, rowCount: rows.length, since, until });
      } catch (e) {
        await pgPool.query(`UPDATE media_accounts SET last_sync_error=$1 WHERE tenant_id=$2 AND advertiser_id=$3 AND channel='cafe24'`, [e?.message||'동기화 실패', tenantId, advertiserId]).catch(()=>{});
        return sendJson(res, 502, { error: e?.message || '카페24 동기화에 실패했습니다.' });
      }
    }

    // ── 네이버 스마트스토어 연결 테스트 ─────────────────────────────────────
    if (req.method === 'POST' && pathname === '/api/integrations/naver-store/test') {
      const body = await readJson(req);
      const clientId = cleanText(body.clientId || '', 200);
      const clientSecret = cleanText(body.clientSecret || '', 200);
      if (!clientId || !clientSecret) return sendJson(res, 400, { error: 'Application ID와 Secret Key가 필요합니다.' });
      try {
        const info = await naverStoreTestConnection(clientId, clientSecret);
        return sendJson(res, 200, { ok: true, ...info });
      } catch (e) { return sendJson(res, 502, { error: e?.message || '네이버 스마트스토어 연결에 실패했습니다.' }); }
    }
    ...
    if (req.method === 'POST' && pathname === '/api/integrations/sync-naver-store') {
      ...
      const since = sinceDate.toISOString().slice(0, 10);
      try {
        const rows = await naverStoreFetchDailyStats(clientId, clientSecret, since, until);
        if (rows.length) {
          await pgPool.query(
            `INSERT INTO daily_metrics (tenant_id, advertiser_id, channel, date, impressions, clicks, spend, db_count, purchases, revenue, add_to_cart, complete_registration, initiate_checkout, unconfirmed_count)
             SELECT $1,$2,'naver_store',d,0,0,0,0,pur,rev,0,0,0,0
             FROM UNNEST($3::date[],$4::bigint[],$5::numeric[]) AS t(d,pur,rev)
             ON CONFLICT (advertiser_id, channel, date) DO UPDATE SET
               purchases=EXCLUDED.purchases, revenue=EXCLUDED.revenue, updated_at=now()`,
            [tenantId, advertiserId, rows.map(r=>r.date), rows.map(r=>r.purchases), rows.map(r=>r.revenue)]
        ...
        await pgPool.query(`UPDATE media_accounts SET last_synced_at=now(), last_row_count=$1, status='connected' WHERE tenant_id=$2 AND advertiser_id=$3 AND channel='naver_store'`, [rows.length, tenantId, advertiserId]);
        return sendJson(res, 200, { ok: true, days, rowCount: rows.length, since, until });
      } catch (e) {
        await pgPool.query(`UPDATE media_accounts SET last_sync_error=$1 WHERE tenant_id=$2 AND advertiser_id=$3 AND channel='naver_store'`, [e?.message||'동기화 실패', tenantId, advertiserId]).catch(()=>{});
        return sendJson(res, 502, { error: e?.message || '네이버 스마트스토어 동기화에 실패했습니다.' });
      }
    }
    ...
    if (req.method === 'GET' && pathname === '/api/creative-metrics') {
      const tenantId = await getCurrentTenantId(); const filters=await parseMetricQuery(); if(!filters)return true;const db=(await pgReadDb(tenantId, filters));const names=advertiserNameMap(db);const source=filterMetricRows(db.creativeDailyMetrics,filters);const grouped=new Map();
      // 일별 CTR 추이를 계산하기 위한 날짜별 데이터 맵
      const dailyData=new Map();
      for(const row of source){const key=`${row.advertiserId}|${row.channel}|${row.adId}`;const cur=grouped.get(key)||{advertiserId:row.advertiserId,advertiserName:names.get(String(row.advertiserId))||String(row.advertiserId),channel:row.channel,campaignId:row.campaignId||'',campaignName:row.campaignName||'',campaignType:row.campaignType||'',adgroupId:row.adgroupId||'',adgroupName:row.adgroupName||'',adId:row.adId,adName:row.adName,thumbnailUrl:row.thumbnailUrl||null,mediaType:row.mediaType||null,carouselImages:row.carouselImages||null,title:row.title||'',body:row.body||'',description:row.description||'',cta:row.cta||'',impressions:0,clicks:0,spend:0,dbCount:0,purchases:0,addToCart:0,completeRegistration:0,initiateCheckout:0,revenue:0};cur.impressions+=metricNumber(row.impressions);cur.clicks+=metricNumber(row.clicks);cur.spend+=metricNumber(row.spend);cur.dbCount+=metricNumber(row.dbCount);cur.purchases+=metricNumber(row.purchases);cur.addToCart+=metricNumber(row.addToCart);cur.completeRegistration+=metricNumber(row.completeRegistration);cur.initiateCheckout+=metricNumber(row.initiateCheckout);cur.revenue+=metricNumber(row.revenue);grouped.set(key,cur);
        // 일별 데이터 누적 (CTR 추이용)
        if(!dailyData.has(key))dailyData.set(key,new Map());
        const dayMap=dailyData.get(key);const date=String(row.date||'').slice(0,10);
        if(date){const existing=dayMap.get(date)||{impressions:0,clicks:0,spend:0};existing.impressions+=metricNumber(row.impressions);existing.clicks+=metricNumber(row.clicks);existing.spend+=metricNumber(row.spend);dayMap.set(date,existing);}
      }
      // 일별 CTR 배열을 날짜순으로 정렬해 trend 필드에 추가합니다.
      const rows=Array.from(grouped.values()).map(cur=>{
        const dayMap=dailyData.get(`${cur.advertiserId}|${cur.channel}|${cur.adId}`);
        const trend=dayMap?[...dayMap.entries()].sort((a,b)=>a[0].localeCompare(b[0])).map(([,v])=>v.impressions>0?parseFloat((v.clicks/v.impressions*100).toFixed(3)):0):[];
        const spendTrend=dayMap?[...dayMap.entries()].sort((a,b)=>a[0].localeCompare(b[0])).map(([,v])=>v.spend):[];
        return {...withDerived(cur),trend,spendTrend,days:trend.length};
    ...
    if (req.method === 'PUT' && pathname === '/api/campaigns') {
      ...
      const targetType = ['adset', 'creative', 'keyword'].includes(body.targetType) ? body.targetType : 'campaign';
...
async function runCampaignAutomationRule(rule) {
  ...
  await pgPool.query(
    ...
    [rule.tenant_id, rule.id, rule.advertiser_id, `[자동화] ${rule.name}`, `${{ campaign: '캠페인', adset: '광고 세트', creative: '소재', keyword: '키워드' }[targetType]} "${cfg.targetName || cfg.campaignName || targetId}"을(를) ${cfg.action === 'on' ? 'ON' : 'OFF'} 처리했습니다.`]
  );
  return result;
}
