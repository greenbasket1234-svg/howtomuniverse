...
function grade(row:KeywordMetricRow){const conv=row.dbCount+(row.purchases||0);if(row.spend>0&&row.clicks>=10&&conv===0)return{label:'비용 낭비',tone:'danger' as const};if(row.impressions>=100&&row.clicks===0)return{label:'제외 후보',tone:'warning' as const};if((row.cvr||0)>=5&&conv>=2)return{label:'고성과',tone:'success' as const};return{label:'안정',tone:'neutral' as const}}
...
export function SearchAdKeywordDetailPage({channel}:{channel:ChannelKey}){
  ...
  return <div><Link className="breadcrumb-back" to={`/search-ads/${channel}`}>← 광고주 목록으로</Link><PageHeader title={`${found.name} · ${CHANNEL_LABEL[channel]} 검색광고`} description="실제 keyword_daily_metrics를 조회하는 읽기 전용 성과 화면입니다." action={<a className="btn secondary" href={CHANNEL_URL[channel]} target="_blank" rel="noreferrer">광고센터 <ExternalLink size={14}/></a>}/><MetricsDateBar/>
    ...
    <div className="card" style={{padding:0}}><div className="table-scroll"><table className="data-table"><thead><tr><th>키워드</th><th>캠페인</th><th>광고그룹</th><th className="num">노출</th><th className="num">클릭</th><th className="num">CTR</th><th className="num">CPC</th><th className="num">CPM</th><th className="num">광고비</th><th className="num">전환</th><th className="num">전환율</th><th className="num">CPA</th><th className="num">전환매출</th><th className="num">ROAS</th><th>분석</th></tr></thead><tbody>{rows.map((r,i)=>{const g=grade(r);return <tr key={`${r.keywordId||r.keyword}-${i}`}><td><b>{r.keyword}</b></td><td>{r.campaignName||'-'}</td><td>{r.adgroupId||'-'}</td><td className="num">{r.impressions.toLocaleString()}</td><td className="num">{r.clicks.toLocaleString()}</td><td className="num">{pct(r.clicks,r.impressions)}</td><td className="num">{r.clicks?won(r.spend/r.clicks):'-'}</td><td className="num">{r.impressions?won(r.spend/r.impressions*1000):'-'}</td><td className="num">{won(r.spend)}</td><td className="num">{(r.dbCount+(r.purchases||0)).toLocaleString()}{(r.dbCount>0&&(r.purchases||0)>0)&&<><br/><small className="table-cell-note">DB {r.dbCount.toLocaleString()} · 구매 {(r.purchases||0).toLocaleString()}</small></>}</td><td className="num">{pct(r.dbCount+(r.purchases||0),r.clicks)}</td><td className="num">{(r.dbCount+(r.purchases||0))?won(r.spend/(r.dbCount+(r.purchases||0))):'-'}</td><td className="num">{r.revenue?won(r.revenue):'-'}</td><td className="num">{r.revenue?`${(r.roas||0).toFixed(1)}%`:'-'}</td><td><Badge tone={g.tone}>{g.label}</Badge></td></tr>})}{!metrics.loading&&!rows.length&&<tr><td colSpan={15} style={{textAlign:'center',padding:30,color:'var(--text-muted)'}}>선택 기간에 실제 키워드 데이터가 없습니다.</td></tr>}</tbody></table></div></div>
