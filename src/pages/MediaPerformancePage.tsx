...
export function MediaPerformancePage(){
  ...
  const accounts=useMemo(()=>{
    if(activeMedia==='전체 비교') return [];
    return [...new Set(MOCK_CAMPAIGNS.filter(c=>normalizeCampaignMedia(c.platform)===activeMedia).map(c=>c.accountName))];
  },[activeMedia]);
  const selectedCampaigns=useMemo(()=>MOCK_CAMPAIGNS.filter(c=>(activeMedia==='전체 비교'||normalizeCampaignMedia(c.platform)===activeMedia)&&(!advertiser||ADVERTISERS.find(a=>a.id===c.advertiserId)?.name===advertiser)&&(!account||c.accountName===account)),[activeMedia,advertiser,account]);
  ...
  return <div className="media-analysis-page">
    ...
    <section className="map-panel"><div className="map-panel-head"><div><h2>매체별 상세 성과</h2><p>검색·정렬 후 상세 분석으로 드릴다운할 수 있습니다.</p></div><div className="map-table-tools"><input value={tableQuery} onChange={e=>setTableQuery(e.target.value)} placeholder="매체 검색"/><select value={verdictFilter} onChange={e=>setVerdictFilter(e.target.value)}><option value="">판정 전체</option><option>효율적 확대</option><option>성장 기회</option><option>안정</option><option>유지</option><option>비효율 증가</option><option>축소 검토</option></select><select value={sortKey} onChange={e=>setSortKey(e.target.value as typeof sortKey)}><option value="health">종합 점수</option><option value="spend">광고비</option><option value="leads">DB</option><option value="cpa">CPA</option><option value="roas">ROAS</option></select><button onClick={()=>setSortDir(v=>v==='asc'?'desc':'asc')}>{sortDir==='asc'?'오름차순':'내림차순'}</button><button onClick={()=>{setTableQuery('');setVerdictFilter('');setSortKey('health');setSortDir('desc')}}>표 필터 초기화</button></div></div><div className="table-scroll"><table className="map-table"><thead><tr><th>매체</th><th>광고비</th><th>노출</th><th>클릭</th><th>CTR</th><th>CPC</th><th>DB</th><th>CPA</th><th>매출</th><th>ROAS</th><th>KPI 달성</th><th>종합</th><th>판정</th><th>상세</th></tr></thead><tbody>{filteredTableRows.map(row=><tr key={row.name}><td><span className="map-media-mark" style={{background:MEDIA_COLORS[row.name]}}/>{row.name}</td><td>{formatMetric('spend',row.current.spend)}</td><td>{formatMetric('impressions',row.current.impressions)}</td><td>{formatMetric('clicks',row.current.clicks)}</td><td>{formatMetric('ctr',row.current.ctr)}</td><td>{row.current.clicks?formatMetric('cpa',{...row.current,cpa:row.current.spend/row.current.clicks}.cpa):'-'}</td><td>{formatMetric('leads',row.current.leads)}</td><td>{formatMetric('cpa',row.current.cpa)}</td><td>{formatMetric('revenue',row.current.revenue)}</td><td>{formatMetric('roas',row.current.roas)}</td><td>{row.kpiAchievement===undefined?'-':`${row.kpiAchievement.toFixed(0)}%`}</td><td><b>{row.healthScore}점</b></td><td><span className={`map-verdict ${row.budgetVerdict.includes('비효율')||row.budgetVerdict.includes('축소')?'bad':row.budgetVerdict.includes('확대')||row.budgetVerdict.includes('기회')?'good':'neutral'}`}>{row.budgetVerdict}</span></td><td><button className="map-link" onClick={()=>chooseMedia(row.name)}>상세 분석</button></td></tr>)}</tbody></table></div></section>
...
function MultiMediaTrend({rows,data,start,end,advertiser,metric}:{rows:ReturnType<typeof buildMediaComparison>;data:ReturnType<typeof loadPerformanceDataset>;start:string;end:string;advertiser:string;metric:PerformanceMetric}){
...
function SelectedMediaDetail({media,summary,reportType,campaigns}:{media:string;summary:ReturnType<typeof derived>;reportType?:string;campaigns:typeof MOCK_CAMPAIGNS}){
