...
export function MediaPerformancePage({ embedded = false, defaultAdvertiser = '' }: MediaPerformancePageProps){
  ...
  <th>구매 전환</th><th>장바구니 담기</th><th>회원가입</th><th>CVR</th>
  ...
  <th>판정</th><th>상세</th>
</tr></thead><tbody>{filteredTableRows.map(row=><tr key={row.name}><td><span className="map-media-mark" style={{background:MEDIA_COLORS[row.name]}}/>{row.name}</td><td className="metric-emphasis">{formatMetric('spend',row.current.spend)}</td><td>{formatMetric('impressions',row.current.impressions)}</td><td>{formatMetric('clicks',row.current.clicks)}</td><td>{formatMetric('ctr',row.current.ctr)}</td><td>{row.current.clicks?formatMetric('cpa',{...row.current,cpa:row.current.spend/row.current.clicks}.cpa):'-'}</td><td><b>{formatMetric('leads',row.current.leads)}</b></td><td>{formatMetric('leads',row.current.purchases??0)}</td><td>{formatMetric('leads',row.current.addToCart??0)}</td><td>{formatMetric('leads',row.current.completeRegistration??0)}</td><td>{row.current.clicks?formatMetric('cvr',row.current.cvr):'-'}</td><td>{formatMetric('cpa',row.current.cpa)}</td><td>{formatMetric('revenue',row.current.revenue)}</td><td className={row.current.roas>=200?'metric-positive':row.current.roas>0&&row.current.roas<100?'metric-negative':''}>{formatMetric('roas',row.current.roas)}</td><td>{row.kpiAchievement===undefined?'-':`${row.kpiAchievement.toFixed(0)}%`}</td><td><b>{row.healthScore}점</b></td><td><span className={`map-verdict ${row.budgetVerdict.includes('비효율')||row.budgetVerdict.includes('축소')?'bad':row.budgetVerdict.includes('확대')||row.budgetVerdict.includes('기회')?'good':'neutral'}`}>{row.budgetVerdict}</span></td><td><button className="map-link" onClick={()=>chooseMedia(row.name)}>상세 분석</button></td></tr>)}</tbody></table></div></section>
