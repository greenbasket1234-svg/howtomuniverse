...
export function IntegratedPerformanceAnalysisPage(){
  ...
  return <>
    ...
    <section className="insight-live-grid"><article className="card"><h3>매체별 성과</h3><div className="table-scroll"><table className="ops-table"><thead><tr>
      ...
      <th className="sortable-th" onClick={()=>mediaSort.toggleSort('dbCount')}>DB 전환{mediaSort.arrow('dbCount')}</th>
      <th className="sortable-th" onClick={()=>mediaSort.toggleSort('unconfirmed')} title="상세 리포트가 아직 없는 시점(주로 오늘)이라 확정 분류 못 한 전환">미확인 ⓘ{mediaSort.arrow('unconfirmed')}</th>
      <th className="sortable-th" onClick={()=>mediaSort.toggleSort('purchases')}>구매 전환{mediaSort.arrow('purchases')}</th>
      <th className="sortable-th" onClick={()=>mediaSort.toggleSort('addToCart')}>장바구니 담기{mediaSort.arrow('addToCart')}</th>
      <th className="sortable-th" onClick={()=>mediaSort.toggleSort('completeRegistration')}>회원가입{mediaSort.arrow('completeRegistration')}</th>
    ...
    <article className="card"><h3>광고주별 성과</h3><div className="table-scroll"><table className="ops-table"><thead><tr>
      ...
      <th className="sortable-th" onClick={()=>advSort.toggleSort('dbCount')}>DB 전환{advSort.arrow('dbCount')}</th>
      <th className="sortable-th" onClick={()=>advSort.toggleSort('unconfirmed')} title="상세 리포트가 아직 없는 시점(주로 오늘)이라 확정 분류 못 한 전환">미확인 ⓘ{advSort.arrow('unconfirmed')}</th>
      <th className="sortable-th" onClick={()=>advSort.toggleSort('purchases')}>구매 전환{advSort.arrow('purchases')}</th>
      <th className="sortable-th" onClick={()=>advSort.toggleSort('addToCart')}>장바구니 담기{advSort.arrow('addToCart')}</th>
      <th className="sortable-th" onClick={()=>advSort.toggleSort('completeRegistration')}>회원가입{advSort.arrow('completeRegistration')}</th>
    ...
    </tr></thead><tbody>{advertisers.length?advertisers.map(r=><tr key={r.id}><td><b>{r.name}</b></td><td className="metric-emphasis">{formatMetric('spend',r.spend)}</td><td>{r.clicks.toLocaleString()}</td><td>{r.dbCount.toLocaleString()}</td><td>{r.unconfirmed?r.unconfirmed.toLocaleString():'-'}</td><td>{r.purchases.toLocaleString()}</td><td>{r.addToCart?r.addToCart.toLocaleString():'-'}</td><td>{r.completeRegistration?r.completeRegistration.toLocaleString():'-'}</td><td>{(r.dbCount+r.purchases)?formatMetric('cpa',r.cpa):'-'}</td><td>{formatMetric('revenue',r.revenue)}</td><td className={roasClass(r.roas)}>{r.spend?`${r.roas.toFixed(0)}%`:'-'}</td></tr>):<tr><td colSpan={11} className="empty-cell">광고주 데이터가 없습니다.</td></tr>}</tbody></table></div></article></section>
