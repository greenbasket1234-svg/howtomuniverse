...
export function IntegratedPerformanceAnalysisPage(){
  ...
  return <>
    ...
    </tr></thead><tbody>{loading?<tr><td colSpan={12} className="empty-cell">불러오는 중...</td></tr>:media.length?media.map(r=><tr key={r.channel}><td><ChannelTag channel={r.channel}/></td><td className="metric-emphasis">{formatMetric('spend',r.spend)}</td><td>{r.impressions.toLocaleString()}</td><td>{r.clicks.toLocaleString()}</td><td>{r.ctr.toFixed(2)}%</td><td>{r.dbCount.toLocaleString()}</td><td>{r.purchases.toLocaleString()}</td><td>{r.addToCart?r.addToCart.toLocaleString():'-'}</td><td>{r.completeRegistration?r.completeRegistration.toLocaleString():'-'}</td><td>{(r.dbCount+r.purchases)?formatMetric('cpa',r.cpa):'-'}</td><td>{formatMetric('revenue',r.revenue)}</td><td className={roasClass(r.roas)}>{r.spend?`${r.roas.toFixed(0)}%`:'-'}</td></tr>):<tr><td colSpan={12} className="empty-cell">선택 기간에 실제 매체 데이터가 없습니다.</td></tr>}</tbody></table></div></article>
    ...
    </tr></thead><tbody>{advertisers.length?advertisers.map(r=><tr key={r.id}><td><b>{r.name}</b></td><td className="metric-emphasis">{formatMetric('spend',r.spend)}</td><td>{r.clicks.toLocaleString()}</td><td>{r.dbCount.toLocaleString()}</td><td>{r.purchases.toLocaleString()}</td><td>{r.addToCart?r.addToCart.toLocaleString():'-'}</td><td>{r.completeRegistration?r.completeRegistration.toLocaleString():'-'}</td><td>{(r.dbCount+r.purchases)?formatMetric('cpa',r.cpa):'-'}</td><td>{formatMetric('revenue',r.revenue)}</td><td className={roasClass(r.roas)}>{r.spend?`${r.roas.toFixed(0)}%`:'-'}</td></tr>):<tr><td colSpan={10} className="empty-cell">광고주 데이터가 없습니다.</td></tr>}</tbody></table></div></article></section>
