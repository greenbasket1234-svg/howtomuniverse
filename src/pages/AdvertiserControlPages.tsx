...
import { getSubscription, loadSubscriptions } from '../utils/subscriptionStore';
...
export function AdvertiserWorkspaceDashboardPage(){
  ...
  const subscription=getSubscription(advertiserId);
  ...
  return <div className="ctrl-page"><PageHeader title="광고주 대시보드" description="광고주 단위의 구독·권한·담당자·공유·승인 상태를 한 화면에서 확인합니다."/>
    ...
    <div className="ctrl-grid-2">
      ...
      <ControlPanel title="계약·구독" description="결제와 분리된 프론트 구독 설정입니다." actions={<Link className="btn secondary" to="/advertisers/subscription">구독 설정</Link>}><div className="ctrl-info-list"><div><span>상품</span><b>{subscription?.planName||'미설정'}</b></div><div><span>상태</span><ControlStatus tone={subscription? 'info':'neutral'}>{subscription?.status||'결제 미연동'}</ControlStatus></div><div><span>시작일</span><b>{date(subscription?.startedAt)}</b></div><div><span>갱신 예정</span><b>{date(subscription?.renewsAt)}</b></div></div></ControlPanel>
    </div>
...
export function AdvertiserPortalPreviewPage(){
  useControlRevision();const rows=advertisers();const [advertiserId,setAdvertiserId]=useState(rows[0]?.id||'');const advertiser=rows.find(a=>a.id===advertiserId);const workspace=ensureAdvertiserWorkspaces().find(w=>w.advertiserId===advertiserId);const features=loadAdvertiserFeatureAccess().filter(f=>f.advertiserId===advertiserId&&f.enabled);const explicit=new Map(features.map(f=>[f.featureKey,f.enabled]));const fallback=(key:string)=>explicit.has(key)?!!explicit.get(key):['dashboard.view','reports.view','assets.view'].includes(key);const shared=loadSharedAssets().filter(s=>s.advertiserId===advertiserId&&s.status==='shared').length;const approvals=loadApprovalRequests().filter(a=>a.advertiserId===advertiserId&&a.status==='pending').length;const subscription=getSubscription(advertiserId);
