  ...
  return <>
    ...
    {detail&&<ModalPortal onClose={()=>setDetail(null)} wide>
      ...
      <div style={{display:'flex',alignItems:'center',gap:8}}>
        ...
        {detail.adId&&<button className="btn secondary" onClick={()=>setScheduleTarget(detail)}><CalendarClock size={14}/> 일정 설정</button>}
        <button className="icon-btn" onClick={()=>setDetail(null)}><X size={18}/></button>
      </div>
      </div>
      {(()=>{const myRules=rulesForCreative(detail);return myRules.length>0&&<p className="muted-text" style={{marginTop:-6,marginBottom:12}}>등록된 예약: {myRules.map(x=>`${ruleScheduleSummary(x)} · ${x.config?.action==='on'?'켜기':'끄기'}${x.enabled?'':'(중지됨)'}`).join(' / ')}</p>;})()}
