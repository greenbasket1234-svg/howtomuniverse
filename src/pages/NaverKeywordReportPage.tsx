...
export function NaverKeywordReportPage(){
  ...
  return <>
    ...
    {channel==='naver'&&new Set(visible.map(r=>r.keywordId||r.keyword)).size>=2000&&<div className="card" style={{color:'#a35b00',background:'#fff7e6',borderColor:'#ffe4b3',marginBottom:12,padding:'10px 14px',fontSize:13}}>네이버는 키워드가 많으면 동기화 시간이 오래 걸려 <b>상위 2000개까지만</b> 수집됩니다. 캠페인 분석·통합 홈의 합계보다 이 화면 합계가 적을 수 있어요.</div>}
    ...
    <section className="card media-report-card" style={{borderTop:`3px solid ${current.color}`}}>
      ...
      <div className="table-scroll"><table className="media-report-table"><thead><tr>
        ...
        {sortedVisible.map((r,i)=><tr key={`${r.keywordId||r.keyword}-${i}`}><td>{r.advertiserName||r.advertiserId}</td><td>{r.campaignName||'-'}</td><td>{r.campaignType&&r.campaignType!=='-'?<CampaignTypeTag type={r.campaignType}/>:'-'}</td><td>{r.adgroupName||r.adgroupId||'-'}</td><td><b>{r.keyword}</b></td><td className="metric-emphasis">{won(r.spend)}</td><td>{r.impressions.toLocaleString()}</td><td>{r.impressions?won(r.cpm):'-'}</td><td>{r.clicks.toLocaleString()}</td><td>{pct(r.ctr||0)}</td><td>{r.clicks?won(r.cpc):'-'}</td><td>{r.dbCount.toLocaleString()}</td><td>{r.unconfirmed?r.unconfirmed.toLocaleString():'-'}</td><td>{(r.purchases||0).toLocaleString()}</td><td>{r.addToCart?r.addToCart.toLocaleString():'-'}</td><td>{r.completeRegistration?r.completeRegistration.toLocaleString():'-'}</td><td>{pct(r.cvr||0)}</td><td>{r.totalConversions?won(r.cpa):'-'}</td><td>{r.revenue?won(r.revenue):'-'}</td><td className={r.revenue&&(r.roas||0)>=200?'metric-positive':r.revenue&&(r.roas||0)<100?'metric-negative':''}>{r.revenue?pct(r.roas||0):'-'}</td></tr>)}
    ...
    <div className="keyword-analysis-cards">
      <div className="card"><div className="card-title">고성과 캠페인</div>{highCampaigns.length?highCampaigns.map((c,i)=><p key={i} className="analysis-item analysis-item-high"><span className="analysis-name-block"><small className="analysis-advertiser">{c.advertiserName}</small><b className="analysis-target analysis-target-high">{c.campaignName}</b></span><span className="analysis-metrics">{c.revenue>0?`ROAS ${(c.revenue/c.spend*100).toFixed(0)}%`:''} · CVR {c.clicks?(c.dbCount/c.clicks*100).toFixed(1):'0.0'}% · 전환 {c.dbCount}건 · 클릭 {c.clicks.toLocaleString()}</span></p>):<p className="muted-text">선택 기간에 뚜렷한 고성과 캠페인이 없습니다.</p>}</div>
      <div className="card"><div className="card-title">저성과 캠페인</div>{lowCampaigns.length?lowCampaigns.map((c,i)=><p key={i} className="analysis-item analysis-item-low"><span className="analysis-name-block"><small className="analysis-advertiser">{c.advertiserName}</small><b className="analysis-target analysis-target-low">{c.campaignName}</b></span><span className="analysis-metrics">광고비 {won(c.spend)} · 전환 {c.dbCount}건 · CVR {c.clicks?(c.dbCount/c.clicks*100).toFixed(1):'0.0'}%{c.revenue>0?` · ROAS ${(c.revenue/c.spend*100).toFixed(0)}%`:''}</span></p>):<p className="muted-text">선택 기간에 뚜렷한 저성과 캠페인이 없습니다.</p>}</div>
      <div className="card"><div className="card-title">고성과 키워드</div>{highKeywords.length?highKeywords.map((r,i)=><p key={i} className="analysis-item analysis-item-high"><span className="analysis-name-block"><small className="analysis-advertiser">{r.advertiserName}</small><b className="analysis-target analysis-target-high">{r.keyword}</b></span><span className="analysis-metrics">{r.revenue>0?`ROAS ${(r.revenue/r.spend*100).toFixed(0)}%`:''} · CVR {r.clicks?(r.dbCount/r.clicks*100).toFixed(1):'0.0'}% · 전환 {r.dbCount}건 · 클릭 {r.clicks.toLocaleString()}</span></p>):<p className="muted-text">선택 기간에 뚜렷한 고성과 키워드가 없습니다.</p>}</div>
      <div className="card"><div className="card-title">저성과 키워드</div>{lowKeywords.length?lowKeywords.map((r,i)=><p key={i} className="analysis-item analysis-item-low"><span className="analysis-name-block"><small className="analysis-advertiser">{r.advertiserName}</small><b className="analysis-target analysis-target-low">{r.keyword}</b></span><span className="analysis-metrics">광고비 {won(r.spend)} · 전환 {r.dbCount}건 · CVR {r.clicks?(r.dbCount/r.clicks*100).toFixed(1):'0.0'}%{r.revenue>0?` · ROAS ${(r.revenue/r.spend*100).toFixed(0)}%`:''}</span></p>):<p className="muted-text">선택 기간에 뚜렷한 저성과 키워드가 없습니다.</p>}</div>
    </div>
