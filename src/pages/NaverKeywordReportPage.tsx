  ...
  return <>
    ...
    <section className="card media-report-card" style={{borderTop:`3px solid ${current.color}`}}>
      ...
      <div className="table-scroll"><table className="media-report-table"><thead><tr>
        ...
        {sortedVisible.map((r,i)=><tr key={`${r.keywordId||r.keyword}-${i}`}><td>{r.advertiserName||r.advertiserId}</td><td>{r.campaignName||'-'}</td><td>{r.campaignType&&r.campaignType!=='-'?<CampaignTypeTag type={r.campaignType}/>:'-'}</td><td>{r.adgroupName||r.adgroupId||'-'}</td><td><b>{r.keyword}</b></td><td className="metric-emphasis">{won(r.spend)}</td><td>{r.impressions.toLocaleString()}</td><td>{r.impressions?won(r.cpm):'-'}</td><td>{r.clicks.toLocaleString()}</td><td>{pct(r.ctr||0)}</td><td>{r.clicks?won(r.cpc):'-'}</td><td><b>{(r.totalConversions??r.dbCount).toLocaleString()}</b>{(r.dbCount>0||r.purchases>0)&&<small style={{display:'block',color:'var(--text-muted)'}}>{[r.dbCount>0&&`DB ${r.dbCount.toLocaleString()}`,r.purchases>0&&`구매 ${r.purchases.toLocaleString()}`].filter(Boolean).join(' · ')}</small>}</td><td>{pct(r.cvr||0)}</td><td>{r.totalConversions?won(r.cpa):'-'}</td><td>{r.revenue?won(r.revenue):'-'}</td><td className={r.revenue&&(r.roas||0)>=200?'metric-positive':r.revenue&&(r.roas||0)<100?'metric-negative':''}>{r.revenue?pct(r.roas||0):'-'}</td></tr>)}
