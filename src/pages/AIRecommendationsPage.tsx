...
function RecommendationCard({rank,recommendation}:{rank:number;recommendation:Recommendation}){const rec=recommendation;const detailHref=`/insights/ai-recommendations/${encodeURIComponent(rec.recommendationId)}`;return <div className="card recommendation-card"><div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:12}}><div style={{display:'flex',gap:10,alignItems:'flex-start'}}><span style={{fontWeight:700,color:'var(--text-secondary)'}}>{rank}</span><div><div style={{display:'flex',gap:6,alignItems:'center',marginBottom:4}}><Badge tone={TYPE_TONE[rec.type]}>{typeIcon(rec.type)} {TYPE_LABEL[rec.type]}</Badge><Badge tone={priorityTone(rec.priorityLabel)}>{rec.priorityLabel}</Badge></div><div style={{fontWeight:600}}>{rec.advertiserName}{rec.mediaName?` · ${rec.mediaName}`:''} · {rec.targetLabel}{rec.mediaName==='naver'&&rec.campaignType&&<> <CampaignTypeTag type={rec.campaignType}/></>}</div>{(rec.targetType==='creative'||rec.targetType==='keyword')&&(rec.campaignName||rec.adgroupName)&&<div style={{fontSize:12,color:'var(--text-secondary)',marginTop:2}}>{rec.campaignName&&<>캠페인 &quot;{rec.campaignName}&quot;</>}{rec.campaignName&&rec.adgroupName?' · ':''}{rec.adgroupName&&<>{rec.mediaName==='meta'?'광고세트':'광고그룹'} &quot;{rec.adgroupName}&quot;</>}</div>}</div></div><span style={{fontSize:12.5,color:'var(--text-secondary)'}}>{rec.confidence.label}</span></div><div style={{display:'flex',flexWrap:'wrap',gap:16,margin:'10px 0',fontSize:13.5}}>{rec.metrics.map(m=><span key={m.label}><strong>{m.label}</strong> {m.detail}</span>)}</div><p style={{margin:'8px 0'}}>{rec.summary}</p><div style={{display:'flex',gap:8,flexWrap:'wrap'}}><Link className="btn" to={detailHref}>근거 보기</Link>{rec.suggestedActions.map(a=><Link key={a.label} className="btn" to={a.to}>{a.label}</Link>)}</div></div>}
