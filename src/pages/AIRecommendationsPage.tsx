...
export function AIRecommendationsPage(){
  ...
  async function handleDeepDive(){if(!recommendations.length)return;setAiStatus('loading');try{const context=buildAIRecommendationContext(advertiser||'전체','현재 선택 기간',recommendations.slice(0,10));await requestAIDeepDive(context);setAiStatus('idle')}catch(error){if(error instanceof AIGatewayNotImplementedError){setAiStatus('not_ready');return}setAiStatus('not_ready')}}
  return <div>
    ...
    <div className="card" style={{display:'flex',flexWrap:'wrap',gap:10,alignItems:'center'}}>
      ...
      <button type="button" className="btn btn-primary" onClick={handleDeepDive} disabled={aiStatus==='loading'} style={{marginLeft:'auto'}}><Sparkles size={14}/> AI 심층 분석</button>
    </div>
    {error&&<div className="card" style={{borderColor:'#fecaca',color:'#b91c1c'}}>{error}</div>}
    {aiStatus==='not_ready'&&<div className="card" style={{borderColor:'var(--warning,#d97706)'}}>AI 심층 분석 외부 API가 아직 연결되지 않았습니다. 아래 추천 자체는 Mock이 아니라 실제 HOWTOM Metrics 데이터 기반 규칙 분석입니다.</div>}
