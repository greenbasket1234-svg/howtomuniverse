...
import { automationApi } from '../automation/automationApi';
...
export function AdminControlDashboardPage(){
  useRevision();const users=loadControlUsers();const [advertisers]=useAdvertisers();const assets=loadAssets(true);const audits=loadAuditEvents();
  const [subCount,setSubCount]=useState<number|null>(null);const [usageCount,setUsageCount]=useState<number|null>(null);
  const [jobCount,setJobCount]=useState<number|null>(null);const [failedCount,setFailedCount]=useState<number|null>(null);
...
function ExecutionsAdmin(){
  const [runs,setRuns]=useState<import('../automation/automationApi').AutomationRun[]>([]);
  useEffect(()=>{ automationApi.runs.list().then(setRuns).catch(()=>setRuns([])); },[]);
  return <ControlPanel title="전체 작업 실행 기록" description="AI 자동화 → 실행 기록과 같은 automation_runs 저장소를 관리자 관점에서 조회합니다."><div className="ctrl-table-wrap"><table className="ctrl-table"><thead><tr><th>시각</th><th>작업</th><th>광고주</th><th>상태</th></tr></thead><tbody>{runs.slice(0,100).map(r=><tr key={r.id}><td>{new Date(r.started_at).toLocaleString('ko-KR')}</td><td><b>{r.rule_name||r.rule_type}</b></td><td>{r.advertiser_id||'-'}</td><td><ControlStatus tone={r.status==='success'?'success':r.status==='failed'?'danger':'warning'}>{r.status}</ControlStatus></td></tr>)}</tbody></table>{!runs.length&&<ControlEmpty>실행 기록이 없습니다.</ControlEmpty>}</div></ControlPanel>
}
