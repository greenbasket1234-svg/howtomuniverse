...
export function CampaignAnalysisPage(){
  ...
  const visible=useMemo(()=>rows.filter(r=>matchesAdvertiserFilter(r.advertiserName||r.advertiserId,filterValue)&&(channel==='all'||r.channel===channel)&&(`${r.campaignName} ${r.advertiserName||''}`).toLowerCase().includes(query.toLowerCase())).sort((a,b)=>b.spend-a.spend),[rows,filterValue,channel,query]);
  ...
  return <>
    ...
    <section className="campaign-live-grid"><article className="card"><div className="table-scroll"><table className="ops-table"><thead><tr><th>캠페인</th><th>매체</th><th>광고주</th><th>광고비</th><th>노출</th><th>클릭</th><th>CTR</th><th>전환</th><th>CPA</th><th>매출</th><th>ROAS</th></tr></thead><tbody>{loading?<tr><td colSpan={11} className="empty-cell">불러오는 중...</td></tr>:visible.length===0?<tr><td colSpan={11} className="empty-cell">선택 기간에 실제 캠페인 성과가 없습니다.</td></tr>:visible.map(r=><tr key={`${r.advertiserId}-${r.channel}-${r.campaignId}`} className={current?.campaignId===r.campaignId?'selected-row':''} onClick={()=>setSelected(r.campaignId)}><td><b>{r.campaignName}</b><small>{r.campaignId}</small></td><td>{r.channel==='meta'?'Meta':r.channel==='naver'?'네이버':r.channel}</td><td>{r.advertiserName}</td><td>{won(r.spend)}</td><td>{r.impressions.toLocaleString()}</td><td>{r.clicks.toLocaleString()}</td><td>{Number(r.ctr||0).toFixed(2)}%</td><td>{r.dbCount.toLocaleString()}</td><td>{r.dbCount?won(r.cpa||0):'-'}</td><td>{won(r.revenue)}</td><td>{r.spend?`${Number(r.roas||0).toFixed(0)}%`:'-'}</td></tr>)}</tbody></table></div></article>
