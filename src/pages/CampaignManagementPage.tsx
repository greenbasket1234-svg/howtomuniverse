  ...
  return <div>
    ...
    <div className="card" style={{padding:0}}>
      <div className="table-scroll fixed-scroll-box"><table className="data-table campaign-table"><thead><tr><th>매체</th><th style={{cursor:'pointer'}} onClick={()=>toggleSort('name')}>캠페인{sortArrow('name')}</th><th>광고그룹·세트</th><th>광고계정</th><th className="num" style={{cursor:'pointer'}} onClick={()=>toggleSort('budget')}>예산{sortArrow('budget')}</th><th>운영 기간</th><th>자동 일정</th><th style={{cursor:'pointer'}} onClick={()=>toggleSort('status')}>상태{sortArrow('status')}</th><th>최근 동기화</th><th>작업</th></tr></thead><tbody>
        {filtered.map(r=>{const myRules=rulesFor(r);const isEditingBudget=editingBudget===r.id;const isAdset=r.level==='adset'||r.level==='adgroup';const isExpanded=expandedCampaigns.has(r.id);const isLoadingAg=adgroupLoading.has(r.id);const adgroups=adgroupMap[r.id]||[];const supportsAdgroups=r.platform==='naver'&&!isAdset;return <><tr key={r.id} style={isAdset?{background:'#f8fafc',borderLeft:'3px solid #e0e7ff'}:{}}><td><ChannelTag channel={r.platform}/></td><td><div style={{display:'flex',alignItems:'center',gap:4}}>{isAdset&&<span style={{fontSize:9,background:'#e0e7ff',color:'#4338ca',borderRadius:3,padding:'1px 4px',flexShrink:0}}>{r.level==='adset'?'광고세트':'광고그룹'}</span>}{supportsAdgroups&&<button className="icon-btn" style={{width:20,height:20,flexShrink:0}} title={isExpanded?'접기':'광고그룹 펼치기'} onClick={()=>toggleExpand(r)}>{isLoadingAg?<Loader size={12}/>:isExpanded?<ChevronDown size={13}/>:<ChevronRight size={13}/>}</button>}<strong style={isAdset?{fontWeight:500,color:'#374151'}:{}}>{r.name}</strong>{r.campaignType&&!isAdset&&<> <CampaignTypeTag type={r.campaignType}/></>}</div></td><td style={{fontSize:12,color: isAdset?'#1e293b':'#94a3b8'}}>{r.adgroupName||(supportsAdgroups?<span style={{color:'#cbd5e1',fontSize:11}}>▷ 펼쳐서 확인</span>:'—')}</td><td>{r.accountName}</td>
