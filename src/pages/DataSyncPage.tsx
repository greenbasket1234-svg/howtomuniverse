...
export function DataSyncPage() {
  ...
  return <div>
    ...
    <section className="card" style={{marginTop:16,padding:0}}><div style={{padding:'16px 18px 8px'}}><h3 style={{margin:0,fontSize:15}}>Sync 검증 로그</h3><p className="footnote">Meta·네이버 관리자 원천 합계와 HOWTOM 중앙 Metrics 저장값을 같은 광고계정·같은 기간으로 대조합니다. 광고비뿐 아니라 노출·클릭·전환·전환매출까지 함께 검증합니다.</p></div><div className="table-scroll"><table className="data-table"><thead><tr><th>검증 시각</th><th>광고주</th><th>매체/계정</th><th>기간</th><th className="num">원천/저장 광고비</th><th className="num">원천/저장 노출</th><th className="num">원천/저장 클릭</th><th className="num">원천/저장 전환</th><th className="num">원천/저장 매출</th><th>결과</th></tr></thead><tbody>{validationLogs.filter(v=>matchesAdvertiserFilter(v.advertiserName||rows.find(r=>r.advertiserId===v.advertiserId)?.advertiserName||'',filterValue)).map(v=><tr key={v.id}><td>{new Date(v.createdAt).toLocaleString('ko-KR')}</td><td>{v.advertiserName||v.advertiserId}</td><td><ChannelTag channel={v.channel}/><small style={{display:'block',marginTop:2}}>{v.accountId||'-'}</small></td><td>{v.since} ~ {v.until}</td><td className="num">₩{Math.round(v.source?.spend||0).toLocaleString()} / ₩{Math.round(v.stored?.spend||0).toLocaleString()}</td><td className="num">{Math.round(v.source?.impressions||0).toLocaleString()} / {Math.round(v.stored?.impressions||0).toLocaleString()}</td><td className="num">{Math.round(v.source?.clicks||0).toLocaleString()} / {Math.round(v.stored?.clicks||0).toLocaleString()}</td><td className="num">{Math.round(v.source?.dbCount||0).toLocaleString()} / {Math.round(v.stored?.dbCount||0).toLocaleString()}</td><td className="num">₩{Math.round(v.source?.revenue||0).toLocaleString()} / ₩{Math.round(v.stored?.revenue||0).toLocaleString()}</td><td><span className={`status-pill ${v.ok?'success':'danger'}`}>{v.ok?'일치':'불일치'}</span>{!v.ok&&<small style={{display:'block'}}>광고비 Δ ₩{Math.round(v.delta?.spend||0).toLocaleString()}</small>}</td></tr>)}{!validationLogs.length&&<tr><td colSpan={10} style={{textAlign:'center',padding:28,color:'#9ca3af'}}>아직 Sync 검증 로그가 없습니다. 매체 데이터를 동기화하면 자동으로 생성됩니다.</td></tr>}</tbody></table></div></section>
  </div>;
}
