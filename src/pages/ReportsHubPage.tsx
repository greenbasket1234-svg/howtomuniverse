...
export function ReportsHubPage(){
  ...
  return <div>
    ...
    <div className="dashboard-grid-two">
      <section className="card"><div className="card-title">광고주별 성과</div><div className="table-scroll"><table className="data-table"><thead><tr><th>광고주</th><th className="num">광고비</th><th className="num">클릭</th><th className="num">전환</th><th className="num">CPA</th><th className="num">매출</th><th className="num">ROAS</th></tr></thead><tbody>{byAdvertiser.map(r=><tr key={r.name}><td><b>{r.name}</b></td><td className="num">{won(r.spend)}</td><td className="num">{r.clicks.toLocaleString()}</td><td className="num">{r.dbCount.toLocaleString()}</td><td className="num">{r.dbCount?won(r.spend/r.dbCount):'-'}</td><td className="num">{won(r.revenue)}</td><td className="num">{r.revenue?pct(r.spend?r.revenue/r.spend*100:0):'-'}</td></tr>)}</tbody></table></div></section>
