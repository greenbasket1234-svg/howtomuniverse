...
type Group={name:string;spend:number;impressions:number;clicks:number;dbCount:number;purchases:number;unconfirmed:number;revenue:number};
function groupRows(rows:DailyMetricRow[],key:(r:DailyMetricRow)=>string):Group[]{const map=new Map<string,Group>();for(const r of rows){const k=key(r);const v=map.get(k)||{name:k,spend:0,impressions:0,clicks:0,dbCount:0,purchases:0,unconfirmed:0,revenue:0};v.spend+=r.spend;v.impressions+=r.impressions;v.clicks+=r.clicks;v.dbCount+=r.dbCount;v.purchases+=r.purchases;v.unconfirmed+=(r.unconfirmed||0);v.revenue+=r.revenue;map.set(k,v)}return [...map.values()].sort((a,b)=>b.spend-a.spend)}
...
export function ReportsHubPage(){
  ...
  const total=useMemo(()=>visible.reduce((a,r)=>({spend:a.spend+r.spend,impressions:a.impressions+r.impressions,clicks:a.clicks+r.clicks,dbCount:a.dbCount+r.dbCount,purchases:a.purchases+r.purchases,unconfirmed:a.unconfirmed+(r.unconfirmed||0),revenue:a.revenue+r.revenue}),{spend:0,impressions:0,clicks:0,dbCount:0,purchases:0,unconfirmed:0,revenue:0}),[visible]);
  ...
  return <div>
    ...
    <div className="summary-grid" style={{marginBottom:16}}>
      ...
      <div className="summary-card" title="DB·구매·미확인(당일 잠정치) 전환을 모두 합한 값입니다"><div className="summary-card-label">전환</div><div className="summary-card-value">{(total.dbCount+total.purchases+total.unconfirmed).toLocaleString()}</div></div>
    ...
    <div className="dashboard-grid-two">
      <section className="card"><div className="card-title">광고주별 성과</div><div className="table-scroll"><table className="data-table"><thead><tr><th>광고주</th><th className="num">광고비</th><th className="num">클릭</th><th className="num">전환</th><th className="num">CPA</th><th className="num">매출</th><th className="num">ROAS</th></tr></thead><tbody>{byAdvertiser.map(r=><tr key={r.name}><td><b>{r.name}</b></td><td className="num">{won(r.spend)}</td><td className="num">{r.clicks.toLocaleString()}</td><td className="num">{(r.dbCount+r.purchases+r.unconfirmed).toLocaleString()}</td><td className="num">{(r.dbCount+r.purchases+r.unconfirmed)?won(r.spend/(r.dbCount+r.purchases+r.unconfirmed)):'-'}</td><td className="num">{won(r.revenue)}</td><td className="num">{r.revenue?pct(r.spend?r.revenue/r.spend*100:0):'-'}</td></tr>)}</tbody></table></div></section>
      <section className="card"><div className="card-title">매체별 성과</div><div className="table-scroll"><table className="data-table"><thead><tr><th>매체</th><th className="num">광고비</th><th className="num">노출</th><th className="num">클릭</th><th className="num">CTR</th><th className="num">전환</th><th className="num">ROAS</th></tr></thead><tbody>{byMedia.map(r=><tr key={r.name}><td><b>{r.name}</b></td><td className="num">{won(r.spend)}</td><td className="num">{r.impressions.toLocaleString()}</td><td className="num">{r.clicks.toLocaleString()}</td><td className="num">{pct(r.impressions?r.clicks/r.impressions*100:0)}</td><td className="num">{(r.dbCount+r.purchases+r.unconfirmed).toLocaleString()}</td><td className="num">{r.revenue?pct(r.spend?r.revenue/r.spend*100:0):'-'}</td></tr>)}</tbody></table></div></section>
    </div>
