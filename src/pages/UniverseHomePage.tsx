...
export function UniverseHomePage(){
  ...
  return <div className="universe-home-page universe-home-dashboard home-dashboard-v14">
    ...
    {(overview.addToCart>0||overview.completeRegistration>0||overview.initiateCheckout>0)&&(
      <section className="home-dashboard-kpis home-dashboard-kpis-v14" style={{gridTemplateColumns:'repeat(3,1fr)',marginTop:-4}} aria-label="이커머스 퍼널 전환">
        <Link to="/conversion-funnel" className="home-dashboard-kpi"><span className="home-kpi-label"><Target size={15}/><span>장바구니 담기</span></span><strong>{overview.addToCart.toLocaleString()}</strong><em><small>{periodLabel}</small></em></Link>
        <Link to="/conversion-funnel" className="home-dashboard-kpi"><span className="home-kpi-label"><Target size={15}/><span>회원가입</span></span><strong>{overview.completeRegistration.toLocaleString()}</strong><em><small>{periodLabel}</small></em></Link>
        <Link to="/conversion-funnel" className="home-dashboard-kpi"><span className="home-kpi-label"><Target size={15}/><span>결제시작</span></span><strong>{overview.initiateCheckout.toLocaleString()}</strong><em><small>{periodLabel}</small></em></Link>
      </section>
    )}
