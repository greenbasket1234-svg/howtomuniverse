...
export function UniverseHomePage(){
  ...
  return <div className="universe-home-page universe-home-dashboard home-dashboard-v14">
    ...
    <section className="home-bottom-grid home-bottom-grid-v14">
      <article className="home-dashboard-card compact-table-card"><div className="home-card-head home-card-head-compact"><div><h2>주요 키워드 성과</h2></div></div><div className="home-mini-list">{visibleKeywords.slice(0,5).map(r=><div className="home-mini-row" key={`${r.channel}-${r.keywordId||r.keyword}`}><div className="home-mini-left"><b>{r.keyword}</b><small><ChannelTag channel={r.channel}/>{r.advertiserName}</small></div><div className="home-mini-right"><b className="home-mini-spend">{money(r.spend)}</b><small className="home-mini-sub">전환 <em>{r.dbCount}</em></small></div></div>)}</div>{!keyword.loading&&!visibleKeywords.length&&<div className="home-empty-data small"><Search size={23}/><b>키워드 데이터가 없습니다.</b></div>}</article>
      <article className="home-dashboard-card compact-table-card"><div className="home-card-head home-card-head-compact"><div><h2>주요 소재 성과</h2></div></div><div className="home-mini-list">{visibleCreatives.slice(0,5).map(r=><div className="home-mini-row" key={`${r.channel}-${r.adId}`}><div className="home-mini-left"><b>{r.adName}</b><small><ChannelTag channel={r.channel}/>{r.advertiserName}</small></div><div className="home-mini-right"><b className="home-mini-spend">{money(r.spend)}</b><small className="home-mini-sub">CTR <em>{(r.ctr||0).toFixed(2)}%</em></small></div></div>)}</div>{!creative.loading&&!visibleCreatives.length&&<div className="home-empty-data small"><Sparkles size={23}/><b>소재 데이터가 없습니다.</b></div>}</article>
      <article className="home-dashboard-card compact-table-card"><div className="home-card-head home-card-head-compact"><div><h2>광고주별 성과</h2></div></div><div className="home-mini-list">{byAdvertiser.slice(0,5).map(r=><div className="home-mini-row" key={r.name}><div className="home-mini-left"><b>{r.name}</b><small>전환 <em>{r.db}</em></small></div><div className="home-mini-right"><b className="home-mini-spend">{money(r.spend)}</b><small className="home-mini-sub">ROAS <em className={r.revenue&&r.spend&&(r.revenue/r.spend*100)>=100?'positive':''}>{r.revenue&&r.spend?(r.revenue/r.spend*100).toFixed(0)+'%':'-'}</em></small></div></div>)}</div>{!daily.loading&&!byAdvertiser.length&&<div className="home-empty-data small"><TrendingUp size={23}/><b>광고 성과 데이터가 없습니다.</b></div>}</article>
