...
export function CreativeDetailPage(){
  ...
  return <div className="creative-fullscreen-page">
    ...
    <MetricsDateBar/>
    <div className="creative-fullscreen-hero"><section className="card creative-fullscreen-media-card"><div className="creative-fullscreen-section-title">실제 매체 소재</div><div className="creative-fullscreen-media-stage image">{creative.thumbnailUrl?<img src={creative.thumbnailUrl} alt={creative.adName}/>:<span className="creative-fullscreen-media-symbol">소재 미리보기 없음</span>}</div>{(creative.title||creative.body||creative.description)&&<div className="creative-fullscreen-copy"><span>매체 문구</span>{creative.title&&<p><b>{creative.title}</b></p>}{creative.body&&<p>{creative.body}</p>}{creative.description&&<p>{creative.description}</p>}</div>}</section><section className="card creative-fullscreen-info-card"><div className="card-title">선택 기간 실제 성과</div><div className="detail-kpi-grid"><div><span>광고비</span><b>{won(creative.spend)}</b></div><div><span>노출</span><b>{creative.impressions.toLocaleString()}</b></div><div><span>클릭</span><b>{creative.clicks.toLocaleString()}</b></div><div><span>CTR</span><b>{Number(creative.ctr||0).toFixed(2)}%</b></div><div><span>CPC</span><b>{won(creative.cpc||0)}</b></div><div><span>CPM</span><b>{won(creative.cpm||0)}</b></div><div><span>전환</span><b>{(creative.dbCount+(creative.purchases||0)).toLocaleString()}</b>{((creative.dbCount>0)||((creative.purchases||0)>0))&&<small className="detail-kpi-sub">{creative.dbCount>0&&`DB ${creative.dbCount.toLocaleString()}`}{creative.dbCount>0&&(creative.purchases||0)>0&&' · '}{(creative.purchases||0)>0&&`구매 ${(creative.purchases||0).toLocaleString()}`}</small>}</div><div><span>CPA</span><b>{(creative.dbCount+(creative.purchases||0))?won(creative.spend/(creative.dbCount+(creative.purchases||0))):'-'}</b></div><div><span>전환매출</span><b>{won(creative.revenue)}</b></div><div><span>ROAS</span><b>{creative.spend?`${Number(creative.roas||0).toFixed(0)}%`:'-'}</b></div></div><dl className="kv-grid creative-fullscreen-kv"><dt>광고 ID</dt><dd>{creative.adId}</dd><dt>캠페인 ID</dt><dd>{creative.campaignId||'-'}</dd><dt>CTA</dt><dd>{creative.cta||'-'}</dd></dl></section></div>
  </div>;
}
