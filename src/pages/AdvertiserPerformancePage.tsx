...
function kpiDisplay(row:AdvertiserComparisonRow){if(!row.goal)return '-';return row.goal.goalType==='CPA'?formatMetric('cpa',row.goalTarget??0):formatMetric('roas',row.goalTarget??0);}
...
export function AdvertiserPerformancePage(){
  ...
  return <div className="advertiser-analysis-page">
    ...
    {!selected&&<section className="aap-panel"><div className="aap-panel-head"><div><h2>광고주별 상세 성과</h2><p>상태·KPI·성과를 비교하고 필요한 광고주만 상세 분석합니다.</p></div><div className="aap-table-tools"><input value={tableQuery} onChange={e=>setTableQuery(e.target.value)} placeholder="광고주 검색"/><select value={tableStatus} onChange={e=>setTableStatus(e.target.value)}><option value="">상태 전체</option>{statusOptions.filter(v=>v!=='전체').map(v=><option key={v}>{v}</option>)}</select><select value={sortKey} onChange={e=>setSortKey(e.target.value as typeof sortKey)}><option value="achievement">KPI 달성률</option><option value="health">건강점수</option><option value="risk">위험점수</option><option value="spend">광고비</option><option value="leads">전환</option><option value="revenue">매출</option></select><button onClick={()=>setSortDir(v=>v==='asc'?'desc':'asc')}>{sortDir==='asc'?'오름차순':'내림차순'}</button><button onClick={()=>{setTableQuery('');setTableStatus('');setSortKey('achievement');setSortDir('desc')}}>표 필터 초기화</button></div></div><div className="table-scroll"><table className="aap-table"><thead><tr><th>광고주</th><th>대표 KPI</th><th>목표</th><th>실제</th><th>달성률</th><th>광고비</th><th>전환</th><th>CPA</th><th>매출</th><th>ROAS</th><th>최고 매체</th><th>건강점수</th><th>위험점수</th><th>상태</th><th>상세</th></tr></thead><tbody>{tableRows.map(row=><tr key={row.name}><td><span className="aap-advertiser-dot" style={{background:hashColor(row.name)}}/>{row.name}</td><td>{row.goal?row.primaryLabel:'미설정'}</td><td>{kpiDisplay(row)}</td><td>{actualKpiDisplay(row)}</td><td>{row.kpiAchievement===undefined?'-':`${row.kpiAchievement.toFixed(0)}%`}</td><td>{formatMetric('spend',row.current.spend)}</td><td>{formatMetric('leads',row.current.leads)}</td><td>{formatMetric('cpa',row.current.cpa)}</td><td>{formatMetric('revenue',row.current.revenue)}</td><td>{formatMetric('roas',row.current.roas)}</td><td>{row.topMedia||'-'}</td><td>{row.healthScore===undefined?'평가 보류':`${row.healthScore}점`}</td><td><b className={row.riskScore>=70?'bad':row.riskScore>=50?'warning':'neutral'}>{row.riskScore}점</b></td><td><span className={`aap-status ${tone(row.status)}`}>{row.status}</span></td><td><button className="aap-link" onClick={()=>chooseAdvertiser(row.name)}>상세 분석</button></td></tr>)}</tbody></table></div></section>}
...
function SelectedAdvertiserSummary({row,data,start,end,prevStart,prevEnd,trendMetric,setTrendMetric}:{row:AdvertiserComparisonRow;data:ReturnType<typeof loadPerformanceDataset>;start:string;end:string;prevStart:string;prevEnd:string;trendMetric:PerformanceMetric;setTrendMetric:(m:PerformanceMetric)=>void}){
  ...
  return <>
    ...
    <article className="aap-panel"><div className="aap-panel-head"><div><h2>KPI 목표·예산 진행</h2><p>현재 추세 기준 월말 예상이며 AI 예측이 아닙니다.</p></div><Link to="/kpi-goals">KPI 관리 <ChevronRight size={14}/></Link></div><div className="aap-progress-box"><div><span>대표 KPI</span><b>{row.goal?`${row.primaryLabel} · 목표 ${kpiDisplay(row)}`:'미설정'}</b></div><div><span>현재 달성률</span><b>{row.kpiAchievement===undefined?'-':`${row.kpiAchievement.toFixed(1)}%`}</b></div><div><span>현재 추세 월말 KPI</span><b>{projectedLabel(row)}</b></div><div><span>월 광고 예산</span><b>{row.monthlyBudget?formatMetric('spend',row.monthlyBudget):'미설정'}</b></div><div><span>현재 월 광고비</span><b>{formatMetric('spend',row.monthSpend)}</b></div><div><span>현재 추세 월말 광고비</span><b>{row.monthBudgetProjection===undefined?'-':formatMetric('spend',row.monthBudgetProjection)}</b></div><div className={`forecast ${row.budgetState==='초과 예상'?'bad':row.budgetState==='미소진 예상'?'warning':'good'}`}><span>예산 판정</span><b>{row.budgetState}</b></div></div></article></section>
