...
function SubscriptionSettings(){
  ...
  useEffect(()=>{ subscriptionApi.listUsage().then(rows=>setUsageCount(rows.length)).catch(()=>setUsageCount(0)); },[]);
  return <div className="ctrl-grid-2"><ControlPanel title="HOWTOM 내부 사용 상태" description="구독 한도는 서버에서 실시간으로 강제되지만, 실제 결제(PG)는 아직 연동되지 않았습니다."><div className="ctrl-info-list"><div><span>플랫폼 상태</span><ControlStatus tone="info">Pre-Revenue / Internal</ControlStatus></div><div><span>결제 수단</span><b>미연동</b></div><div><span>이번 달 기록된 AI/콘텐츠 사용 이벤트</span><b>{usageCount==null?'확인 중':`${usageCount}건`}</b></div><div><span>다음 결제</span><b>-</b></div></div></ControlPanel><ControlPanel title="결제 기능" description="실제 정기 결제는 구독 판매가 시작될 때 PG와 서버 Webhook을 연결합니다."><div className="ctrl-disabled-actions"><button className="btn secondary" disabled>상품 변경 (미구현)</button><button className="btn secondary" disabled>결제수단 등록 (미구현)</button></div><p className="ctrl-muted">광고주 계약·구독의 기능 한도 자체는 서버에서 실제로 집행되지만, 청구·결제는 아직 연결되지 않았습니다.</p></ControlPanel></div>
}
