...
import {
  Bell, Bot, Building2, ChevronLeft, Database, HardDrive, KeyRound, Monitor, Save, Settings2, ShieldCheck, Sparkles, UserRound, Users, WalletCards, Workflow,
} from 'lucide-react';
...
function IntegrationSettings(){return <div className="ctrl-grid-2"><ControlPanel title="광고 매체·계정" description="실제 OAuth·토큰 보관은 서버 연결 이후 활성화합니다."><div className="ctrl-info-list"><div><span>현재 방식</span><b>프론트 Connector / Mock Gate</b></div><div><span>Secret 보관</span><b>브라우저 저장 금지</b></div></div><Link className="btn primary" to="/ad-accounts/connections">매체·계정 연동 열기</Link></ControlPanel><ControlPanel title="DB · Google Sheets" description="Pre-Revenue 단계의 핵심 데이터 연결 방식입니다."><div className="ctrl-info-list"><div><span>연동 방식</span><b>Google Sheet / Apps Script</b></div><div><span>유료 서버</span><b>필요 시점까지 미사용 가능</b></div></div><Link className="btn primary" to="/settings/advanced/db-integrations">DB 연동 설정 열기</Link></ControlPanel></div>}
