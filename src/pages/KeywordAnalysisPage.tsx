...
export function KeywordAnalysisBrandListPage(){
  ...
  return <div>
    ...
    <div className="card" style={{padding:0}}><div className="table-scroll"><table className="brand-table"><thead><tr><th>광고주명</th><th>키워드 매체 연동</th><th className="num">실제 키워드 수</th><th></th></tr></thead><tbody>
      {rows.map(advertiser=>{const advRows=metricRows.filter(m=>m.advertiserId===advertiser.id);const conn=(meta?.connections||[]).filter(c=>c.advertiserId===advertiser.id&&['naver','google','kakao','daangn'].includes(c.channel));const uniqueKeywordCount=new Set(advRows.map(r=>r.keywordId||r.keyword)).size;return <tr key={advertiser.id}><td className="brand-name-cell"><Link className="brand-name-link" to={`/keywords/${advertiser.id}/analysis`}>{advertiser.name}</Link></td><td><div className="keyword-platform-badges">{KEYWORD_PLATFORMS.map(platform=>{const channel=PLATFORM_TO_CHANNEL[platform];const c=conn.find(x=>x.channel===channel);return <Badge key={platform} tone={c?.status==='connected'?'success':c?.status==='error'?'danger':'neutral'}>{platform} · {connectionLabel(c?.status)}</Badge>})}</div></td><td className="num">{advRows.length.toLocaleString()}개{uniqueKeywordCount>=2000&&<div title="네이버는 키워드가 많으면 동기화 시간이 오래 걸려 상위 2000개까지만 수집합니다. 캠페인 분석 합계보다 이 화면 합계가 적을 수 있습니다." style={{fontSize:11,color:'#a35b00',marginTop:2}}>⚠ 2000개 제한 도달 가능</div>}</td><td style={{textAlign:'right'}}><Link className="btn btn-primary" to={`/keywords/${advertiser.id}/analysis`}>분석 보기</Link></td></tr>})}
...
export function KeywordAnalysisPage(){
  ...
  const allRows=metricRows.filter(m=>platform==='전체'||CHANNEL_TO_PLATFORM[m.channel]===platform).map((m,i)=>({
    ...
    impressions:m.impressions,clicks:m.clicks,spend:m.spend,conversions:m.dbCount+(m.purchases||0),dbCount:m.dbCount,purchases:m.purchases||0,addToCart:m.addToCart||0,completeRegistration:m.completeRegistration||0,revenue:m.revenue||0,status:'active' as const,
    grade:gradeOf({impressions:m.impressions,clicks:m.clicks,spend:m.spend,conversions:m.dbCount+(m.purchases||0)})
  }));
  ...
  return <div>
    ...
    <div className="card" style={{padding:0}}><div className="table-scroll"><table className="data-table keyword-analysis-table"><thead><tr>
      ...
      {filteredRows.map(row=><tr key={row.id}><td><Badge tone="accent" style={{background:`${getPlatformColor(row.platform)}1a`,color:getPlatformColor(row.platform),border:`1px solid ${getPlatformColor(row.platform)}55`}}>{row.platform}</Badge></td><td><strong>{row.keyword}</strong></td><td>{row.campaign}</td><td><CampaignTypeTag type={row.campaignType}/></td><td>{row.adGroup}</td><td className="num">{row.impressions.toLocaleString()}</td><td className="num">{row.clicks.toLocaleString()}</td><td className="num">{pct(row.clicks,row.impressions)}</td><td className="num">{row.clicks?currency(row.spend/row.clicks):'-'}</td><td className="num">{row.impressions?currency(row.cpm):'-'}</td><td className="num metric-emphasis">{currency(row.spend)}</td><td className="num">{row.dbCount.toLocaleString()}</td><td className="num">{row.purchases.toLocaleString()}</td><td className="num">{row.addToCart?row.addToCart.toLocaleString():'-'}</td><td className="num">{row.completeRegistration?row.completeRegistration.toLocaleString():'-'}</td><td className="num">{pct(row.conversions,row.clicks)}</td><td className="num">{row.conversions?currency(row.spend/row.conversions):'-'}</td><td className="num">{row.revenue?currency(row.revenue):'-'}</td><td className={`num ${row.roas>=200?'metric-positive':row.roas>0&&row.roas<100?'metric-negative':''}`}>{row.revenue?`${row.roas.toFixed(1)}%`:'-'}</td><td><Badge tone={gradeTone[row.grade]}>{gradeLabel[row.grade]}</Badge></td></tr>)}
