...
export const AI_RECOMMENDATION_SYSTEM_RULES = [
  ...
  '각 발견 사항의 제목이나 설명에 반드시 구체적인 광고주명·캠페인명(또는 소재명·키워드명)과 매체명(네이버/메타 등)을 함께 명시한다 - "여러 캠페인이" 같이 뭉뚱그린 표현은 쓰지 않는다.',
  '제공된 목록에 캠페인·소재·키워드가 섞여 있으면, findings가 한 유형에만 쏠리지 않도록 최대한 각 유형에서 최소 1개씩은 다루도록 노력한다.',
] as const;
