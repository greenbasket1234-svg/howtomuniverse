...
export function runWorkflowNow(workflow: AutomationWorkflow) {
  ...
  for (const step of workflow.steps) {
    const result = executeStep(step, workflow);
...
function executeStep(step: WorkflowStep, workflow: AutomationWorkflow): { status: 'success' | 'failed' | 'blocked'; message?: string } {
  ...
  if (step.type === 'ad_copy_generation') {
    ...
    const result = generateAdCopyNow(config); return { status: result?.status === 'success' ? 'success' : result?.status === 'blocked' ? 'blocked' : 'failed', message: result?.error?.message };
  }
