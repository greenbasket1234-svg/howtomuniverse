...
export type PerformanceMetric = 'spend'|'impressions'|'clicks'|'leads'|'revenue'|'ctr'|'cpa'|'roas'|'cpc'|'cvr';
export type PerformancePoint = {
  ...
  unconfirmed?:number;
  addToCart?:number; completeRegistration?:number;
...
export function performanceDatasetFromMetricRows(rows:DailyMetricRow[]):PerformanceDataset{
  ...
  for(const row of rows){
    ...
    const point:PerformancePoint={date:row.date,advertiser,advertiserId:row.advertiserId,reportType,media:normalizeMedia(row.channel),spend:Number(row.spend)||0,impressions:Number(row.impressions)||0,clicks:Number(row.clicks)||0,leads:Number(row.dbCount)||0,revenue:Number(row.revenue)||0,purchases:Number(row.purchases)||0,unconfirmed:Number(row.unconfirmed)||0,addToCart:Number(row.addToCart)||0,completeRegistration:Number(row.completeRegistration)||0};
    media.push(point);
    const key=`${row.date}|${row.advertiserId}`;const cur=totalMap.get(key)??{date:row.date,advertiser,advertiserId:row.advertiserId,reportType,media:'전체',spend:0,impressions:0,clicks:0,leads:0,revenue:0,purchases:0,unconfirmed:0,addToCart:0,completeRegistration:0};
    cur.spend+=point.spend;cur.impressions+=point.impressions;cur.clicks+=point.clicks;cur.leads+=point.leads;cur.revenue+=point.revenue;cur.purchases=(cur.purchases??0)+(point.purchases??0);cur.unconfirmed=(cur.unconfirmed??0)+(point.unconfirmed??0);cur.addToCart=(cur.addToCart??0)+(point.addToCart??0);cur.completeRegistration=(cur.completeRegistration??0)+(point.completeRegistration??0);totalMap.set(key,cur);
  }
...
export function formatMetric(metric:PerformanceMetric,value:number){if(metric==='spend'||metric==='revenue'||metric==='cpa'||metric==='cpc')return value?`₩${Math.round(value).toLocaleString()}`:'-';if(metric==='ctr'||metric==='roas'||metric==='cvr')return `${value.toFixed(value>=100?0:2)}%`;return Math.round(value).toLocaleString();}
