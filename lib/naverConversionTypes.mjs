...
export function classifyNaverConversionType(rawType) {
  ...
  if (!t) return { field: 'unconfirmed', known: false, engagement: false };
  ...
  // 정말 모르는 유형: 확실하지 않은 걸 DB(리드)로 단정 짓지 않고 '미확인' 전환으로 집계합니다
  // (전환을 조용히 버리지는 않되, 정체를 모르는 채로 리드 실적에 섞이지 않게 합니다).
  // known=false를 돌려주므로 호출부에서 반드시 경고 로그를 남깁니다.
  return { field: 'unconfirmed', known: false, engagement: false };
}
