 ...
 * 중요한 제약: Instagram 공식 API는 "게시물 캡션 전체를 자유 키워드로 검색"하는 기능을
 * 제공하지 않습니다 - 오직 "특정 해시태그가 달린 게시물 찾기"만 가능합니다. 그래서 캡션에만
 * 키워드가 있고 해시태그로는 안 달린 게시물은 API 차원에서 애초에 발견할 방법이 없습니다.
 * 이를 최대한 보완하기 위해, 찾은 각 게시물마다 캡션에도 검색어가 실제로 포함되는지 함께
 * 표시해서(captionMatchesQuery) 사용자가 관련도를 판단할 수 있게 합니다.
...
function createInstagramOrganicConnector({ metaGraphGet, metaConfigured }) {
  return {
    ...
    async search({ query, igUserId, limit }) {
      ...
      const hashtag = rawQuery.replace(/^#/, '').replace(/\s+/g, ''); // 해시태그는 공백을 쓸 수 없어 붙여서 만듭니다.
      if (!hashtag) return { items: [], status: 'connected', message: '검색할 키워드를 입력해주세요.' };
      try {
        ...
        if (!hashtagId) {
          return { items: [], status: 'connected', message: `'#${hashtag}' 해시태그를 찾을 수 없습니다. Instagram은 캡션 전체를 자유 검색하는 기능이 없어, 실제로 그 단어가 해시태그로 쓰인 게시물이 있어야만 찾을 수 있습니다.` };
        }
        const fields = 'id,caption,media_type,media_url,permalink,thumbnail_url,timestamp,like_count,comments_count,username';
        const perBucket = String(Math.min(limit || 25, 50));
        // top_media(인기순)와 recent_media(최신순)를 둘 다 모아서, 그 해시태그와 관련된 게시물을 더 폭넓게 찾습니다.
        const [topMedia, recentMedia] = await Promise.all([
          metaGraphGet(`/${hashtagId}/top_media`, { user_id: igUserId, fields, limit: perBucket }),
          metaGraphGet(`/${hashtagId}/recent_media`, { user_id: igUserId, fields, limit: perBucket }).catch(() => ({ data: [] })),
        ]);
        const merged = new Map();
        for (const m of [...(topMedia?.data || []), ...(recentMedia?.data || [])]) merged.set(m.id, m);
        ...
        const items = [...merged.values()].map(m => ({
          ...
          // 캡션에도 검색어가 실제로 들어있는지 표시합니다(해시태그로만 걸린 게 아니라 본문에도 관련 있는지 참고용).
          captionMatchesQuery: !!m.caption && queryWords.some(w => m.caption.toLowerCase().includes(w)),
          views: null,
        ...
        // 캡션에도 검색어가 실제로 포함된 게시물을 우선으로 보여줍니다(해시태그로만 걸린 것보다 더 관련 있을 가능성이 높음).
        items.sort((a, b) => Number(b.captionMatchesQuery) - Number(a.captionMatchesQuery));
        return { items: items.slice(0, Math.min(limit || 25, 50)), status: 'connected' };
      } catch (err) {
